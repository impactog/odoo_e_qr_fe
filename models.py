...
class AccountMove(models.Model):
    ...
    def _compute_json_qr(self):
        for rec in self:
            dict_invoice = ''
            if rec.type in ['out_invoice','out_refund'] and rec.state == 'posted' and rec.l10n_ar_afip_auth_code != '':
            # if rec.type in ['out_invoice','out_refund'] and rec.state == 'posted' and rec.afip_auth_code != '':
                ...
                res = 'N/A'
            rec.l10n_ar_afip_json_qr = res
            if type(dict_invoice) == dict:
                enc = res.encode('utf-8')
                b64 = base64.b64encode(enc)
                rec.l10n_ar_afip_texto_modificado_qr = 'https://www.afip.gob.ar/fe/qr/?p=' + str(b64, 'utf-8')
            else:
                rec.l10n_ar_afip_texto_modificado_qr = 'https://www.afip.gob.ar/fe/qr/?ERROR'
    
    l10n_ar_afip_json_qr = fields.Char("JSON QR AFIP",compute=_compute_json_qr)
    l10n_ar_afip_texto_modificado_qr = fields.Char('Texto Modificado QR',compute=_compute_json_qr)
